...
if(isNil "_veh" || {isNull _veh}) exitWith { life_action_inUse = false; hint "Fahrzeug nichtmehr in der naehe!"; };
...
if(!isNull life_fuelstation_fueltruck) then {
	...
	_txtVeh ctrlSetText format["Gas Pump ($%1 Liter)", life_fuelstation_price];
};
...
if(!isNull life_fuelstation_fueltruck) then { _fitFuel = (life_fuelstation_fueltruck getVariable["fueltruck_fuel",0]) min _fitFuel; };
...
if(isNull life_fuelstation_fueltruck) then {
	_cashFit = ( floor (life_RLP_atmgeld / life_fuelstation_price)) min _fitFuel;
